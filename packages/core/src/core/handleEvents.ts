// Error Stack Parser 是一个 Node.js 库，用于解析 JavaScript 异常的堆栈跟踪信息。
// 它可以帮助你解析错误堆栈字符串，并提供一个方便的 API 来访问堆栈帧的信息。
import ErrorStackParser from 'error-stack-parser';
import {openWhiteScreen, transportData, breadcrumb, resourceTransform, httpTransform, options} from './index';
import {EVENTTYPES, STATUS_CODE} from '@dawei-front/common';
import {getErrorUid, hashMapExist, getTimestamp, parseUrlToObj, unknownToString} from '@dawei-front/utils';
import {ErrorTarget, RouteHistory, HttpData} from '@dawei-front/types';
...
const HandleEvents = {
    ...
    handleError(ev: ErrorTarget): void {
        ...
        if (!target || (ev.target && !ev.target.localName)) {
            ...
            breadcrumb.push({
                type: EVENTTYPES.ERROR,
                category: breadcrumb.getCategory(EVENTTYPES.ERROR), //错误类型
                data: errorData,
                time: getTimestamp(),
                status: STATUS_CODE.ERROR
            });
