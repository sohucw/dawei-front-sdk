import {
    ...
    getLocationHref,
    generateUUID
} from '@dawei-front/utils';
import {SDK_VERSION, EVENTTYPES} from '@dawei-front/common';
import {ReportData, InitOptions} from '@dawei-front/types';
...
export class TransportData {
    ...
    bindOptions(options: InitOptions): void {
        const {url, apikey, beforeDataReport, userId, getUserId, useImgUpload} = options;
        validateOption(apikey, 'apikey', 'string') && (this.apikey = apikey);
        validateOption(url, 'dsn', 'string') && (this.errorDsn = url);
    ...
    async send(data: ReportData) {
        ...
        // 开启录屏，由@dawei-front/recordScreen 插件控制
