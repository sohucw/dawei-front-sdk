import {getFlag, nativeTryCatch, setFlag} from '@dawei-front/utils';
import {ReplaceHandler, ReplaceCallback} from '@dawei-front/types';
...
export function notify(type: EVENTTYPES, data?: any): void {
    ...
    handlers[type]?.forEach((callback) => {
        nativeTryCatch(
            ...
            (e: any) => {
                console.error(`重写事件notify的回调函数发生错误，Type:${type} ${e}`);
            }
        );
    });
}
