import {STATUS_CODE} from '@dawei-front/common';
import {_global, _support} from '@dawei-front/utils';
import {Callback, InitOptions} from '@dawei-front/types';
...
export function openWhiteScreen(callback: Callback, {skeletonProject, whiteBoxElements}: InitOptions) {
    ...
    function sampling() {
        let emptyPoints = 0;
        for (let i = 1; i <= 9; i++) {
            const xElements = document.elementsFromPoint((_global.innerWidth * i) / 10, _global.innerHeight / 2);
            const yElements = document.elementsFromPoint(_global.innerWidth / 2, (_global.innerHeight * i) / 10);
