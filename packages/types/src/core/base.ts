import {EVENTTYPES, STATUS_CODE, USERWAYTYPES} from '@dawei-front/common';
 ...
 * 上报的数据接口
 */
export interface ReportData extends HttpData, ResouceError, LongTask, PerformanceData, MemoryData, CodeError, RecordScreen {
    type: string; // 事件类型
    pageUrl: string; // 页面地址
    time: number; // 发生时间
    uuid: string; // 页面唯一标识
...
export interface UserWayData {
    type: EVENTTYPES; // 事件类型
    category: USERWAYTYPES; // 用户行为类型
